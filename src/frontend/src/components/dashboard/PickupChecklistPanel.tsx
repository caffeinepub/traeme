...
export default function PickupChecklistPanel() {
  ...
  return (
    <Card className="md:col-span-2">
      <CardHeader>
        ...
        <CardDescription>Recent pickup and drop-off records</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
              ...
              <div key={record.id} className="flex justify-between items-center border-b pb-2">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Stop: {record.stopId} | Trip: {record.tripId}
                  </p>
                </div>
                <p className="text-xs text-muted-foreground">
                  {new Date(Number(record.time) / 1000000).toLocaleString()}
                </p>
              </div>
            ))}
